...
const generateAvatarPrompt = ai.definePrompt({
  ...
  prompt: `Create a high-quality, detailed, and clear avatar. The image must be a square (1:1 aspect ratio) and visually impressive, suitable for a profile picture. Base the avatar on the following description: {{{description}}}. Return the image as a data URI.`,
  config: {
    safetySettings: [
      {
...
const generateAvatarFlow = ai.defineFlow(
  ...
  async input => {
    const {media} = await ai.generate({
      ...
      prompt: `Create a high-quality, detailed, and clear avatar. The image must be a square (1:1 aspect ratio) and visually impressive, suitable for a profile picture. Base the avatar on the following description: ${input.description}`,
